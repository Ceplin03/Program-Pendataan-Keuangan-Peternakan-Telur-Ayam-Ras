...
$bulan = ''; // Inisialisasi variabel tahun
$bulan_nama = ''; // Inisialisasi variabel nama bulan

// Periksa apakah form telah disubmit
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['bulan'])) {
    $bulan = $_POST['bulan'];
    $bulan = date('Y-m', strtotime($bulan)); // Format bulan menjadi 'YYYY-MM'

    // Format nama bulan
    $bulan_nama = date('F', strtotime($bulan)); // Mengambil nama bulan penuh
    ...
    $sql = "SELECT id_pendapatan, id_penjualan, id_pengeluaran, tgl_awal, tgl_akhir, tgl_pengeluaran, total_penjualan, total_pengeluaran, total_pendapatan 
            FROM tbl_pendapatan 
            WHERE DATE_FORMAT(tgl_awal, '%Y-%m') = ?";
...
<hr><h5>Laporan Data Keuangan Bulanan</h5><hr></center>
              <table class="table">
                <thead>
                  <tr>
                    <th>ID Pendapatan</th>
                ...
                <tfoot>
                  <tr>
                    <td colspan="8" class="text-right">Total Pendapatan Bulan <?php echo htmlspecialchars($bulan_nama); ?>:</td>
                    <td>Rp. <?php echo number_format($total_pendapatan_bulanan, 2); ?></td>
                  </tr>
